...
export class SearchResource {
  ...
  // Search Operations
  async search<T>(indexId: string, options: z.infer<typeof searchOptionsSchema>): Promise<{
    results: T[];
    total: number;
    similarities?: number[];
  }> {
  ...
  async searchNearText<T>(indexId: string, text: string, options: Omit<z.infer<typeof searchOptionsSchema>, 'query'> = {}): Promise<{
    results: T[];
    total: number;
    similarities?: number[];
  }> {
  ...
  async searchNearVector<T>(indexId: string, vector: number[], options: Omit<z.infer<typeof searchOptionsSchema>, 'query'> = {}): Promise<{
    results: T[];
    total: number;
    similarities?: number[];
  }> {
